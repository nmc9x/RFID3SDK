            ...
            if (tagData != null)
            {
                for (int nIndex = 0; nIndex < tagData.Length; nIndex++)
                {
                        ...
                        tagData[nIndex].OpStatus == ACCESS_OPERATION_STATUS.ACCESS_SUCCESS))
                    {
                        ...
                        lock (m_TagTable.SyncRoot) // lock sync access to hashtable
                        {
                        ...
                        if (isFound)
                        {
                            ...
                            item.SubItems[2].Text = tag.AntennaID.ToString(); // Anten ID
                            item.SubItems[3].Text = count.ToString(); // Read Count 
                            item.SubItems[4].Text = tag.PeakRSSI.ToString(); // RSSI
                            item.SubItems[5].Text = GetPhaseInDegree(tag.PhaseInfo); // Phase
            ...
            RunWorkerCompletedEventArgs connectEventArgs)
        {
            if (m_ConnectionForm.connectionButton.Text == "Connect")
            {
                if (connectEventArgs.Result.ToString() == "Connect Succeed")
                {
                    ...
                    this.connectionStatus.BackgroundImage = Properties.Resources.connected;
                    configureMenuItemsUponConnectDisconnect();
            ...
            else if (m_ConnectionForm.connectionButton.Text == "Disconnect")
            {
                ...
                this.connectionStatus.BackgroundImage = Properties.Resources.disconnected;
